...
from executorch.backends.riscv.common.pipeline_config import RiscvPassPipelineConfig
from executorch.backends.riscv.tosa import TosaSpecification

from executorch.exir.backend.compile_spec_schema import CompileSpec


@dataclass(init=False)
class RiscvCompileSpec(ABC):
    ...
    def _set_compile_specs(
        ...
        pipeline_config: RiscvPassPipelineConfig | None = None,
    ):
    ...
    def from_list(cls, compile_specs: list[CompileSpec]):  # noqa: C901
        ...
        path_for_intermediates: str | None = None
        tosa_debug_mode: RiscvCompileSpec.DebugMode | None = None
        ...
        for spec in compile_specs:
            ...
            if key == RiscvCompileSpec._TOSA_SPEC_KEY:
    ...
    def to_list(self):
        ...
        if len(self.compiler_flags) > 0:
            compile_spec += [
                CompileSpec(
                    RiscvCompileSpec._COMPILE_FLAGS_KEY,
        ...
        compile_spec.append(
            CompileSpec(
                RiscvCompileSpec._OUTPUT_FORMAT_KEY, self.get_output_format().encode()
            )
        )
        ...
        if self.path_for_intermediates is not None:
            compile_spec.append(
                CompileSpec(
                    RiscvCompileSpec._DEBUG_ARTIFACT_KEY,
        ...
        if self.tosa_debug_mode is not None:
            ...
            compile_spec.append(
                CompileSpec(
                    RiscvCompileSpec._DEBUG_MODE_KEY, self.tosa_debug_mode.name.encode()
                )
            )
        ...
        if not self.output_order_workaround:
            compile_spec.append(
                CompileSpec(
                    RiscvCompileSpec._OUTPUT_REORDER_KEY,
        ...
        if self._pipeline_config is not None and not self._pipeline_config.is_default():
            compile_spec.append(
                CompileSpec(
                    RiscvCompileSpec._TRANSFORM_PIPELINE_CONFIG_KEY,
                    self._pipeline_config.serialize(),
                )
            )
        return compile_spec

    def get_pass_pipeline_config(self) -> RiscvPassPipelineConfig:
        """Returns configuration that controls how the Riscv pass pipeline should
        behave.
    ...
    def set_pass_pipeline_config(self, config: RiscvPassPipelineConfig) -> None:
        """Sets the configuration that controls how the Riscv pass pipeline should
        behave. Subclasses may override to tweak defaults for specific targets.

        Args:
            config: The custom RiscvPassPipelineConfig to set.

        """
        self._pipeline_config = config

    def _create_default_pipeline_config(self) -> RiscvPassPipelineConfig:
        config = RiscvPassPipelineConfig()
