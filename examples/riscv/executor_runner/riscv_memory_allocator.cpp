...
RiscvMemoryAllocator::RiscvMemoryAllocator(uint32_t size, uint8_t* base_address)
...
void* RiscvMemoryAllocator::allocate(size_t size, size_t alignment) {
...
size_t RiscvMemoryAllocator::used_size() const {
  return used_;
}

size_t RiscvMemoryAllocator::free_size() const {
  return executorch::runtime::MemoryAllocator::size() - used_;
}

void RiscvMemoryAllocator::reset() {
