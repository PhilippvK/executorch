 ...
 *
 * ET_RISCV_BAREMETAL_METHOD_ALLOCATOR_POOL_SIZE            - Size of memory area
 *                                                          used when setting up
 *                                                          the model
 * ET_RISCV_BAREMETAL_FAST_SCRATCH_TEMP_ALLOCATOR_POOL_SIZE - Size of memory area
 *                                                          used when running
 *                                                          inferences
 */
...
#include "riscv_memory_allocator.h"
#include "riscv_perf_monitor.h"
#include "csr.h"
 ...
 * ET_RISCV_BAREMETAL_SCRATCH_TEMP_ALLOCATOR_POOL_SIZE and
 * ET_RISCV_BAREMETAL_FAST_SCRATCH_TEMP_ALLOCATOR_POOL_SIZE
 */
...
#if defined(ET_RISCV_BAREMETAL_FAST_SCRATCH_TEMP_ALLOCATOR_POOL_SIZE)
extern "C" {
size_t ethosu_fast_scratch_size =
    ET_RISCV_BAREMETAL_FAST_SCRATCH_TEMP_ALLOCATOR_POOL_SIZE;
unsigned char __attribute__((section(".bss.ethosu_scratch"), aligned(16)))
dedicated_sram[ET_RISCV_BAREMETAL_FAST_SCRATCH_TEMP_ALLOCATOR_POOL_SIZE];
...
struct RunnerContext {
  ...
  Box<Program> program;
  Box<RiscvMemoryAllocator> method_allocator;
  Box<RiscvMemoryAllocator> temp_allocator;
...
#endif
#if defined(SEMIHOSTING)
  Box<RiscvMemoryAllocator> input_file_allocator;
