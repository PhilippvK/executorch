...
from executorch.backends.riscv.common.riscv_compile_spec import RiscvCompileSpec
...
_riscv_model_evaluator = None


def _load_riscv_model_evaluator() -> Any:
    """Lazily import riscv_model_evaluator to avoid heavy deps when not evaluating."""
    global _riscv_model_evaluator
    if _riscv_model_evaluator is not None:
        return _riscv_model_evaluator

    try:
        from executorch.backends.riscv.util import riscv_model_evaluator as riscv_eval
    except Exception as exc:
        raise RuntimeError(
            "Unable to run evaluation because riscv_model_evaluator could not be imported. "
            "You probably need to install torchvision or rerun without --evaluate. "
            f"Original import error: {exc}"
        )

    _riscv_model_evaluator = riscv_eval
    return riscv_eval
...
def get_calibration_data(
    ...
    if evaluator_name is not None:
        riscv_eval = _load_riscv_model_evaluator()
        evaluator_data = riscv_eval.evaluator_calibration_data(
...
def get_compile_spec(
    ...
    if debug_mode is not None:
        mode = RiscvCompileSpec.DebugMode[debug_mode.upper()]
...
def get_args():
    ...
    parser.add_argument(
        ...
        required=False,
        default=False,
        help="Flag for producing RiscvBackend delegated model",
    )
    parser.add_argument(
        "--bundleio",
        action="store_true",
    ...
    parser.add_argument(
        ...
        choices=TARGETS,
        help=f"For RiscvBackend delegated models, pick the target, and therefore the instruction set generated. valid targets are {TARGETS}",
    )
    parser.add_argument(
        "-e",
        "--evaluate",
...
if __name__ == "__main__":  # noqa: C901
    ...
    output_name = f"{model_name}" + (
        f"_riscv_delegate_{args.target}"
        if args.delegate is True
        else f"_riscv_{args.target}"
    )
